...
$$B_1L_1 = -B_2L_2$$
Substituting this gives as final precession angle formula (ignoring the $-$ sign)
$$\phi_t = 2c\lambda (B_1 + B_2)\frac{y}{\tan\theta_0} $$
which can be seen to match Equation \eqref{eq:precession-freq} with $\alpha = \frac{(B_1 + B_2)}{\pi\tan\theta_0}$
This gives also the motivation for using two devices: you need at least two to remove this $\theta$-dependency and achieve optimal modulation using a realistic, slightly divergent beam.
\subsubsection{Foil-flippers and Wollaston prisms}
