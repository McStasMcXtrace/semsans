...
$$\frac{A_s(\delta)}{A_b(\delta)} = P(\delta) = e^{G(\delta) - \tau}$$
...
$$f_\lambda(\lambda) = \sqrt{\frac{2}{\pi}}\left(\frac{1}{mk_BT}\right)^{\frac{3}{2}}\frac{h^3}{\lambda^4}e^{-\frac{h^2}{2k_BTm\lambda^2}}$$
...
This can be achieved making the triangle with the stronger field the smallest. 
...
$$\phi = -2c\lambda (B_1 + B_2) \frac{y}{\tan\theta_0}$$
For $\lambda \neq \lambda_0$ this will cause a depolarization as previously shown experimentally \cite{kraan2003}.
...
\end{equation}
the contribution of each separate device being $\phi_i = \frac{2c\lambda B_i y}{\tan\theta_0}$. Table \ref{tab:device-properties} summarizes device characteristics as they appear in literature, these values will be used in this research.

\begin{table}[h!]
	\centering
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		Label & $\theta_0$ (degrees) & $B_{\text{min}}$ (mT) & $B_{\text{max}}$ (mT) & Source \\
		\hline
		Isosceles triangle & 20 & 0.1 & 15 & \cite{kusmin2017} \\
		Wollaston prism & 45 & 0.1 & 63 & \cite{li2021} \\
		Foil flipper & - & 0.3 & 30 & \cite{bouwman2011} \\
		\hline
	\end{tabular}
...
\subsubsection{Effect of $\sigma$ on $\delta$-resolution}
...
$$\Phi_{mc} \propto \Phi_0\int_0^\infty f_\lambda(\lambda)e^{-\frac{1}{2}\left(\frac{\lambda - \lambda_0}{\sigma}\right)^2}d\lambda$$
...
This expression can be used to compute a very coarse estimate for the total sample or detector intensity by substituting the respective distances $d$ from the source.  
