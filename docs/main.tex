...
An alternative and equivalent way of deriving this is through considering the detected $Q$-range, using $Q_{max}$ and $\theta_a$ as described in Section \ref{c3.4} and relating this to precession angle $\phi$ through $\phi = \delta Q$. 
...
In this way, the intuitive notion that the envelope should not become too narrow to measure enough signal can be translated to a concrete limit by choosing $FWHM_{e,min}$. For monochromators with a low $\Delta\lambda$ and a corresponding low $\sigma$ such as PG monochromators, this will in practice not normally be a limit but it can be when using velocity selectors. The chosen value in this work for $FWHM_{e,min}$ is $1 \unit{\milli\meter}$, so about $9.1$\% of the detector height $h_d$ and is somewhat permissive.  
...
Using the above constraints, $\delta$-ranges for the designs can be estimated and these are given in ...
...
$$\Phi(d) \propto \frac{\Phi_{mc}}{4}\frac{R_0^2}{(R_0 + \psi_0 d)^2} = \frac{C_{mc}\Phi_0}{4}\frac{R_0^2}{(R_0 + \psi_0 d)^2}$$
Consider $R = 0.1 \unit{\meter}$ to be the radius of the beamline and let $\phi_0 = 1^\circ$. Using $d = 6 \unit{\meter}$ as estimate for the distance of the sample to the beamline gives a divergence loss factor of about $\frac{R_0^2}{(R_0 + \psi_0 d)^2} = 0.346$. 
